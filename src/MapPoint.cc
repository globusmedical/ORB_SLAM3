...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0),
    mnCorrectedReference(0), mnBAGlobalForKF(0), mnOriginMapId(pMap->GetId()), mpRefKF(pRefKF), mnVisible(1),
    mnFound(1), mbBad(false), mpReplaced(nullptr), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
{
    SetWorldPos(Pos);
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnCorrectedReference(0), mnBAGlobalForKF(0), mnOriginMapId(pMap->GetId()), mpRefKF(nullptr), mnVisible(1),
